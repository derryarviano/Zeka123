...
import {
  ...
  Edit3, Flower2, Home, Leaf, Lightbulb,
  LockKeyhole, Microscope, Palette, Play, Puzzle,
  ShieldCheck, Shapes, Sparkles, Star, UserRound, Volume2, VolumeX, WandSparkles
} from 'lucide-react';
...
function SubjectMark({ id, size = 34 }: { id: SubjectId; size?: number }) {
  if(id==='english') return <span className="subject-mark-inner english abc-only" style={{width:size+12,height:size}} aria-label="Bahasa Inggris"><b>ABC</b></span>;
  const Icon = id === 'indo' ? BookOpenText : id === 'math' ? Shapes : Microscope;
  return <span className={'subject-mark-inner '+id}><Icon size={size} strokeWidth={2.4}/></span>;
}
...
function Onboarding({ onComplete }: { onComplete: (profile: Profile) => void }) {
  ...
  const [assessment, setAssessment] = useState(0); const [assessmentScore,setAssessmentScore]=useState(0);
  const checks = [
...
function CollectionView({ profile, parent }: { profile:Profile; parent:()=>void }) {
  const [panel,setPanel]=useState<'puzzle'|'badges'|null>(null); const [reveal,setReveal]=useState(false);
  const [stars,setStars]=useState(240); const [pieces,setPieces]=useState(3); const [collectionReady,setCollectionReady]=useState(false);
  useEffect(()=>{try{const saved=JSON.parse(localStorage.getItem('zeka-collection')||'{}');if(typeof saved.stars==='number')setStars(saved.stars);if(typeof saved.pieces==='number')setPieces(saved.pieces)}catch{}setCollectionReady(true)},[]);
  useEffect(()=>{if(collectionReady)localStorage.setItem('zeka-collection',JSON.stringify({stars,pieces}))},[stars,pieces,collectionReady]);
...
function ModulesView({ profile, subject, back, play, premium }: {profile:Profile;subject:SubjectId;back:()=>void;play:(index:number)=>void;premium:()=>void}) {
  const s=subjects.find(x=>x.id===subject)!; const modules=moduleNamesFor(subject,profile.grade); const [completed,setCompleted]=useState<string[]>([]);
  useEffect(()=>{try{setCompleted(JSON.parse(localStorage.getItem('zeka-progress')||'{"completed":[]}').completed||[])}catch{}},[]);
  const prefix=`${profile.grade}-${subject}-`; const nextIndex=completed.filter(x=>x.startsWith(prefix)).length;
  return <main className={'path-screen '+s.tone}><header className="path-top"><button onClick={back}><ArrowLeft/></button><Logo/><span>Kelas {profile.grade} · 30 pos</span></header><section className="path-title"><div className="path-emblem"><SubjectMark id={subject} size={43}/></div><div><p className="overline">JALUR PETUALANGAN</p><h1>{s.world}</h1><p>{s.copy}</p></div><img className="path-kobi" src="/assets/zeka-mascot.png" alt="Kobi"/></section><section className="journey"><div className="trail"/>{modules.map((name,i)=>{const open=i<7;const done=completed.includes(`${prefix}${i}`);return <button className={'journey-stop '+(open?'open':'locked')+(done?' completed':i===nextIndex?' current':'')} key={name} onClick={open?()=>play(i):premium}><span className="stop-dot">{open?(done?<Check/>:i+1):<LockKeyhole/>}</span><span><small>{done?'SELESAI':open?'MODUL '+(i+1):'PAKET LENGKAP'}</small><strong>{name}</strong>{i===nextIndex&&open&&<em>Lanjutkan dari sini</em>}</span>{open?<ChevronRight/>:<Crown/>}</button>})}</section></main>;
}
function LessonView({ profile, subject, moduleIndex, exit:_exit, finish }: {profile:Profile;subject:SubjectId;moduleIndex:number;exit:()=>void;finish:()=>void}) {
  ...
  const listen=()=>{if('speechSynthesis' in window){speechSynthesis.cancel();const voice=new SpeechSynthesisUtterance(q.prompt);voice.lang=subject==='english'?'en-US':'id-ID';voice.rate=.85;speechSynthesis.speak(voice)}};
  return <main className="activity-screen"><header className="activity-top"><span className="quiz-lock" aria-hidden="true"><ShieldCheck/></span><div className="progress-rail"><i style={{width:((index+1)/questions.length*100)+'%'}}/></div><b>{index+1}/{questions.length}</b></header><section className="question-card"><div className="kobi-guide"><img src="/assets/zeka-mascot.png" alt="Kobi"/><span>{selected?(right?'Wah, tepat sekali!':'Coba lihat petunjukku.'): 'Aku menemanimu.'}</span></div><p className="overline">{subjects.find(s=>s.id===subject)!.title} · KELAS {profile.grade}</p><h1>{q.prompt}</h1><button className="listen-button" onClick={listen}><Volume2/> Dengarkan</button><div className="choice-stack">{q.options.map(option=><button key={option} className={selected===option?(option===q.answer?'correct':'wrong'):''} onClick={()=>setSelected(option)}><span>{option}</span>{selected===option&&right?<Check/>:<ChevronRight/>}</button>)}</div>{selected&&<div className={'feedback-card '+(right?'success':'gentle')}><Lightbulb/><div><strong>{right?'Hebat! Kamu menemukannya.':'Belum tepat, dan itu tidak apa-apa.'}</strong><p>{right?q.hint:'Petunjuk: '+q.hint}</p></div></div>}<Button className="primary-big" disabled={!right} onClick={next}>{index===questions.length-1?'Selesaikan modul':'Lanjut'} <ChevronRight/></Button></section></main>;
}

function ParentView({profile,edit,back,premium,reset}:{profile:Profile;edit:()=>void;back:()=>void;premium:()=>void;reset:()=>void}) {
  const [done,setDone]=useState<string[]>([]); useEffect(()=>{try{setDone(JSON.parse(localStorage.getItem('zeka-progress')||'{"completed":[]}').completed||[])}catch{}},[]); const subjectDone=(id:string)=>done.filter(x=>x.includes(`-${id}-`)).length; const skills=[['Membaca dan memahami',Math.min(100,subjectDone('indo')*10),subjectDone('indo')?'Makin lancar':'Belum dimulai'],['Berhitung dan logika',Math.min(100,subjectDone('math')*10),subjectDone('math')?'Sedang tumbuh':'Belum dimulai'],['Berani mencoba',Math.min(100,done.length*6),done.length?'Berkembang baik':'Ayo mulai']]; const minutes=done.length*8;
  return <main className="parent-screen"><header><button onClick={back}><ArrowLeft/></button><div><small>RUANG ORANG TUA</small><b>Perkembangan {profile.name}</b></div><button onClick={edit}><Edit3/></button></header><div className="parent-content"><section className="parent-story"><div><p>RINGKASAN MINGGU INI</p><h1>{profile.name} sedang membangun kebiasaan belajar yang baik.</h1><span>{Math.min(5,done.length)} dari 5 hari belajar · {minutes} menit</span></div><div className="progress-orb"><b>{Math.min(100,done.length*10)}%</b><small>target</small></div></section><section className="parent-section"><div className="section-heading"><div><p className="overline">PETA KEMAMPUAN</p><h2>Yang tumbuh dan perlu dibantu</h2></div></div><div className="skill-list">{skills.map(([name,value,status],i)=><div className="skill-row" key={String(name)}><span className={'skill-symbol s'+i}>{i===0?<BookOpenText/>:i===1?<Shapes/>:<Sparkles/>}</span><div><span><strong>{name}</strong><small>{status}</small></span><div className="skill-track"><i style={{width:String(value)+'%'}}/></div></div><b>{value}%</b></div>)}</div></section><section className="home-tip"><span><Home/></span><div><small>COBA DI RUMAH</small><h2>Bermain toko-tokoan 10 menit</h2><p>Ajak {profile.name} menghitung harga dua atau tiga barang.</p></div></section><button className="parent-premium" onClick={premium}><span><Crown/></span><span><strong>Buka seluruh petualangan</strong><small>Modul 8 dan seterusnya di empat mata pelajaran</small></span><ChevronRight/></button><section className="privacy-panel"><ShieldCheck/><div><strong>Privasi keluarga</strong><p>Data belajar tersimpan hanya di perangkat ini. Orang tua dapat menghapus seluruh profil dan progres kapan saja.</p><button onClick={reset}>Hapus data anak</button></div></section></div></main>;
}
...
function BackgroundMusic({ active }: { active: boolean }) {
  const audio=useRef<HTMLAudioElement>(null); const [enabled,setEnabled]=useState(true); const [started,setStarted]=useState(false); const [hidden,setHidden]=useState(false);
  useEffect(()=>{const begin=()=>setStarted(true);window.addEventListener('pointerdown',begin,{once:true});return()=>window.removeEventListener('pointerdown',begin)},[]);
  useEffect(()=>{const player=audio.current;if(!player)return;player.volume=.22;const sync=()=>{if(active&&enabled&&started&&!document.hidden&&document.hasFocus())player.play().catch(()=>{});else player.pause()};sync();document.addEventListener('visibilitychange',sync);window.addEventListener('focus',sync);window.addEventListener('blur',sync);return()=>{document.removeEventListener('visibilitychange',sync);window.removeEventListener('focus',sync);window.removeEventListener('blur',sync)}},[active,enabled,started]);
  return <><audio ref={audio} src="/assets/zeka-bgm.mp3" loop preload="auto"/><div className={'bgm-dock '+(hidden?'hidden':'')}><button className="bgm-hide" onClick={()=>setHidden(!hidden)} aria-label={hidden?'Tampilkan kontrol musik':'Sembunyikan kontrol musik'}><ChevronRight/></button>{!hidden&&<button className="bgm-toggle" onClick={()=>{setStarted(true);setEnabled(value=>!value)}} aria-label={enabled?'Matikan musik':'Nyalakan musik'}>{enabled?<Volume2/>:<VolumeX/>}</button>}</div></>;
}
...
export default function HomePage(){
  ...
  const openSubject=(id:SubjectId)=>{setSubject(id);setView('modules')};
    const finishModule=()=>{let progress:{completed:string[]}={completed:[]};try{progress=JSON.parse(localStorage.getItem('zeka-progress')||'{"completed":[]}')}catch{}const key=`${profile?.grade}-${subject}-${moduleIndex}`;if(!progress.completed.includes(key)){progress.completed.push(key);localStorage.setItem('zeka-progress',JSON.stringify(progress));try{const collection=JSON.parse(localStorage.getItem('zeka-collection')||'{"stars":240,"pieces":3}');collection.stars=(collection.stars??240)+20;localStorage.setItem('zeka-collection',JSON.stringify(collection))}catch{}}setComplete(true)};
  ...
  if(complete)return <><BackgroundMusic active/><main className="finish-screen"><div><div className="kobi-celebrate" role="img" aria-label="Kobi melompat gembira"/><p className="overline">MODUL SELESAI</p><h1>Satu penemuan baru!</h1><p>Kamu sudah menyelesaikan semua tantangan di modul ini. Sekarang mata dan tubuhmu boleh beristirahat.</p><div className="earned"><Star fill="currentColor"/> +20 bintang</div><Button className="primary-big" onClick={()=>{setComplete(false);if(moduleIndex<6){setModuleIndex(moduleIndex+1);setView('lesson')}else setView('modules')}}>Lanjut ke level selanjutnya <ChevronRight/></Button><Button className="secondary-big" onClick={()=>{setComplete(false);setView('home')}}>Kembali ke beranda</Button></div></main></>;
  return <div className="app-shell">
    ...
    {view==='lesson'&&<LessonView profile={profile} subject={subject} moduleIndex={moduleIndex} exit={()=>setView('modules')} finish={finishModule}/>}
    {view==='parent'&&<ParentView profile={profile} edit={()=>setEdit(true)} back={()=>setView('home')} premium={()=>setPremium(true)} reset={()=>{if(window.confirm("Hapus profil dan seluruh progres anak di perangkat ini?")){localStorage.removeItem("zeka-child-profile");localStorage.removeItem("zeka-progress");localStorage.removeItem("zeka-collection");localStorage.removeItem("zeka-assessment-score");setProfile(null);setView("home")}}}/>}
