...
function ModulesView({ profile, subject, back, play, premium }: {profile:Profile;subject:SubjectId;back:()=>void;play:(index:number)=>void;premium:()=>void}) {
  ...
  return <main className={'path-screen '+s.tone}><header className="path-top"><button onClick={back}><ArrowLeft/></button><Logo/><span>Kelas {profile.grade} · 30 pos</span></header><section className="path-title"><div className="path-emblem"><SubjectMark id={subject} size={43}/></div><div><p className="overline">JALUR PETUALANGAN</p><h1>{s.world}</h1><p>{s.copy}</p></div><img className="path-kobi" src="/assets/zeka-mascot.png" alt="Kobi"/></section><section className="journey"><div className="trail"/>{modules.map((name,i)=>{const open=i<7;return <button className={'journey-stop '+(open?'open':'locked')+(i===0?' current':'')} key={name} onClick={open?()=>play(i):premium}><span className="stop-dot">{open?i+1:<LockKeyhole/>}</span><span><small>{open?'MODUL '+(i+1):'PAKET LENGKAP'}</small><strong>{name}</strong>{i===0&&<em>Lanjutkan dari sini</em>}</span>{open?<ChevronRight/>:<Crown/>}</button>})}</section></main>;
}
